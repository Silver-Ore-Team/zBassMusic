...
namespace NH::Bass
{
	void Channel::Play(const MusicDef& music, const MusicFile* file)
	{
		if (m_Stream > 0)
		{
			BASS.BASS_ChannelFree(m_Stream);
		}

		m_Music = music;
		m_Stream = BASS.BASS_StreamCreateFile(true, file->Buffer.data(), 0, file->Buffer.size(), 0);
		if (!m_Stream)
		{
			log->Error("Could not create stream: {0}\n  error: {1}\n  at {2}:{3}",
					m_Music.Filename, Engine::ErrorCodeToString(BASS.BASS_ErrorGetCode()),
					__FILE__, __LINE__);
			return;
		}

		BASS.BASS_ChannelStart(m_Stream);
		...
		{
			BASS.BASS_ChannelSetAttribute(m_Stream, BASS_ATTRIB_VOL, 0.0f);
			BASS.BASS_ChannelSlideAttribute(m_Stream, BASS_ATTRIB_VOL, m_Music.Volume, m_Music.StartTransition.Duration);
		}
		else
		{
			BASS.BASS_ChannelSetAttribute(m_Stream, BASS_ATTRIB_VOL, m_Music.Volume);
		}

		if (m_Music.Loop)
		{
			BASS.BASS_ChannelFlags(m_Stream, BASS_SAMPLE_LOOP, BASS_SAMPLE_LOOP);
			log->Trace("Loop set {0}", music.Filename);
		}

		if (!Options->ForceDisableReverb && m_Music.Effects.Reverb)
		{
			HFX fx = BASS.BASS_ChannelSetFX(m_Stream, BASS_FX_DX8_REVERB, 1);
			BASS_DX8_REVERB params{ 0, m_Music.Effects.ReverbMix, m_Music.Effects.ReverbTime, 0.001f };
			if (!BASS.BASS_FXSetParameters(fx, (void*)&params))
			{
		...
		{
			const QWORD length = BASS.BASS_ChannelGetLength(m_Stream, BASS_POS_BYTE);
			const QWORD transitionBytes = BASS.BASS_ChannelSeconds2Bytes(m_Stream, m_Music.EndTransition.Duration / 1000.0f);
			const QWORD offset = length - transitionBytes;
			BASS.BASS_ChannelSetSync(m_Stream, BASS_SYNC_POS, offset, OnTransitionSync, this);
			log->Trace("SyncTransitionPos set: {0}", m_Music.Filename);
		}

		BASS.BASS_ChannelSetSync(m_Stream, BASS_SYNC_END, 0, OnEndSync, this);
	...
	double Channel::CurrentPosition() const
	{
		if (m_Stream > 0)
		{
			return BASS.BASS_ChannelBytes2Seconds(m_Stream, BASS.BASS_ChannelGetPosition(m_Stream, BASS_POS_BYTE));
		}
		return -1;
	}
	...
	double Channel::CurrentLength() const
	{
		if (m_Stream > 0)
		{
			return BASS.BASS_ChannelBytes2Seconds(m_Stream, BASS.BASS_ChannelGetLength(m_Stream, BASS_POS_BYTE));
		}
		return -1;
	}
	...
	void Channel::Stop()
	{
		if (m_Stream > 0)
		{
			...
			{
				BASS.BASS_ChannelSlideAttribute(m_Stream, BASS_ATTRIB_VOL, 0.0f, m_Music.EndTransition.Duration);
				BASS.BASS_ChannelSetSync(m_Stream, BASS_SYNC_SLIDE, 0, OnVolumeSlideSync, this);
	...
	void Channel::OnVolumeSlideSync(HSYNC, DWORD channel, DWORD data, void* userData)
	{
		...
		{
			float volume;
			BASS.BASS_ChannelGetAttribute(channel, BASS_ATTRIB_VOL, &volume);
